...
public class PropertyCalculator {
    ...
    public float calculateAdsTime(AttachmentCacheProperty cacheProperty) {
        Float originalValue = cacheProperty.getCache(TaczModifierIds.ADS_TIME);
    ...
    public float calculateAmmoSpeed(AttachmentCacheProperty cacheProperty) {
        Float originalValue = cacheProperty.getCache(TaczModifierIds.AMMO_SPEED);
    ...
    public float calculateArmorIgnore(AttachmentCacheProperty cacheProperty) {
        Float originalValue = cacheProperty.getCache(TaczModifierIds.ARMOR_IGNORE);
    ...
    public float calculateEffectiveRange(AttachmentCacheProperty cacheProperty) {
        Float originalValue = cacheProperty.getCache(TaczModifierIds.EFFECTIVE_RANGE);
    ...
    public float calculateHeadshotMultiplier(AttachmentCacheProperty cacheProperty) {
        Float originalValue = cacheProperty.getCache(TaczModifierIds.HEAD_SHOT);
    ...
    public float calculateKnockback(AttachmentCacheProperty cacheProperty) {
        Float originalValue = cacheProperty.getCache(TaczModifierIds.KNOCKBACK);
    ...
    public float calculateWeight(AttachmentCacheProperty cacheProperty) {
        Float originalValue = cacheProperty.getCache(TaczModifierIds.WEIGHT);
    ...
    public int calculatePierce(AttachmentCacheProperty cacheProperty) {
        Integer originalValue = cacheProperty.getCache(TaczModifierIds.PIERCE);
    ...
    public int calculateRoundsPerMinute(AttachmentCacheProperty cacheProperty) {
        Integer originalValue = cacheProperty.getCache(TaczModifierIds.ROUNDS_PER_MINUTE);
    ...
    public MoveSpeed calculateMoveSpeed(AttachmentCacheProperty cacheProperty) {
        MoveSpeed originalMoveSpeed = cacheProperty.getCache(TaczModifierIds.MOVEMENT_SPEED);
        ...
        // 通过反射创建新的MoveSpeed对象并设置值
        try {
            MoveSpeed result = new MoveSpeed();
            
            // 使用反射设置私有字段值
            java.lang.reflect.Field baseField = MoveSpeed.class.getDeclaredField("baseMultiplier");
            baseField.setAccessible(true);
            baseField.set(result, originalMoveSpeed.getBaseMultiplier() * playerAttributeFactor);
            
            java.lang.reflect.Field aimField = MoveSpeed.class.getDeclaredField("aimMultiplier");
            aimField.setAccessible(true);
            aimField.set(result, originalMoveSpeed.getAimMultiplier() * playerAttributeFactor);
            
            java.lang.reflect.Field reloadField = MoveSpeed.class.getDeclaredField("reloadMultiplier");
            reloadField.setAccessible(true);
            reloadField.set(result, originalMoveSpeed.getReloadMultiplier() * playerAttributeFactor);
    ...
    private MoveSpeed createDefaultMoveSpeed() {
        ...
        try {
            ...
            java.lang.reflect.Field baseField = MoveSpeed.class.getDeclaredField("baseMultiplier");
            baseField.setAccessible(true);
            baseField.set(result, 0.0f);
            
            java.lang.reflect.Field aimField = MoveSpeed.class.getDeclaredField("aimMultiplier");
            aimField.setAccessible(true);
            aimField.set(result, 0.0f);
            
            java.lang.reflect.Field reloadField = MoveSpeed.class.getDeclaredField("reloadMultiplier");
            reloadField.setAccessible(true);
            reloadField.set(result, 0.0f);
    ...
    public LinkedList<ExtraDamage.DistanceDamagePair> calculateDamage(AttachmentCacheProperty cacheProperty) {
        LinkedList<ExtraDamage.DistanceDamagePair> originalDamage = cacheProperty.getCache(TaczModifierIds.DAMAGE);
    ...
    public Map<InaccuracyType, Float> calculateInaccuracy(AttachmentCacheProperty cacheProperty) {
        Map<InaccuracyType, Float> originalInaccuracy = cacheProperty.getCache(TaczModifierIds.INACCURACY);
    ...
    public ParameterizedCachePair<Float, Float> calculateRecoil(AttachmentCacheProperty cacheProperty) {
        ParameterizedCachePair<Float, Float> originalRecoil = cacheProperty.getCache(TaczModifierIds.RECOIL);
    ...
    public Pair<Integer, Boolean> calculateSilence(AttachmentCacheProperty cacheProperty) {
        Pair<Integer, Boolean> originalSilence = cacheProperty.getCache(TaczModifierIds.SILENCE);
    ...
    public Ignite calculateIgnite(AttachmentCacheProperty cacheProperty) {
        Ignite originalIgnite = cacheProperty.getCache(TaczModifierIds.IGNITE);
    ...
    public ExplosionData createExplosionData(AttachmentCacheProperty cacheProperty) {
        ExplosionData originalExplosion = cacheProperty.getCache(TaczModifierIds.EXPLOSION);
        ...
        List<Boolean> knockbackValues = List.of(originalExplosion.isKnockback(), playerAttributeKnockback);
        ...
        List<Boolean> destroyBlockValues = List.of(originalExplosion.isDestroyBlock(), playerAttributeDestroyBlock);
        ...
        int delay = (int) Math.round(originalExplosion.getDelay() * playerAttributeFactorDelay);
