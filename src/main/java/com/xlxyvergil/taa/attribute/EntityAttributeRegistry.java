...
public class EntityAttributeRegistry {
    ...
    public static final DeferredRegister<Attribute> ATTRIBUTES = DeferredRegister.create(Registries.ATTRIBUTE, TaczAttributeAdd.MODID);
    ...
    public static final DeferredHolder<Attribute, Attribute> EXPLOSION_KNOCKBACK = ATTRIBUTES.register("explosion_knockbacknew", 
        () -> new RangedAttribute("attribute.name.taa.explosion_knockbacknew", 1.0D, 0.01D, 3.0D));
    ...
    public static void registerAttributes(EntityAttributeModificationEvent event) {
        ...
        event.getTypes().forEach(type -> {
            // 添加所有枪械相关属性
            event.add(type, ADS_TIME);
            event.add(type, AMMO_SPEED);
            event.add(type, ARMOR_IGNORE);
            event.add(type, EFFECTIVE_RANGE);
            event.add(type, EXPLOSION_RADIUS);
            event.add(type, EXPLOSION_DAMAGE);
            event.add(type, EXPLOSION_KNOCKBACK);
            event.add(type, EXPLOSION_DESTROY_BLOCK);
            event.add(type, EXPLOSION_DELAY);
            event.add(type, EXPLOSION_ENABLED);
            event.add(type, MOVE_SPEED);
            event.add(type, HEADSHOT_MULTIPLIER);
            event.add(type, IGNITE);
            event.add(type, INACCURACY);
            event.add(type, INACCURACY_STAND);
            event.add(type, INACCURACY_MOVE);
            event.add(type, INACCURACY_SNEAK);
            event.add(type, INACCURACY_LIE);
            event.add(type, INACCURACY_AIM);
            event.add(type, KNOCKBACK);
            event.add(type, PIERCE);
            event.add(type, RECOIL);
            event.add(type, RECOIL_PITCH);
            event.add(type, RECOIL_YAW);
            event.add(type, ROUNDS_PER_MINUTE);
            event.add(type, SILENCE);
            event.add(type, WEIGHT);
            
            // 添加枪械伤害属性
            event.add(type, BULLET_GUNDAMAGE);
            event.add(type, BULLET_GUNDAMAGE_PISTOL);
            event.add(type, BULLET_GUNDAMAGE_RIFLE);
            event.add(type, BULLET_GUNDAMAGE_SHOTGUN);
            event.add(type, BULLET_GUNDAMAGE_SNIPER);
            event.add(type, BULLET_GUNDAMAGE_SMG);
            event.add(type, BULLET_GUNDAMAGE_LMG);
            event.add(type, BULLET_GUNDAMAGE_LAUNCHER);
            
            // 添加新属性
            event.add(type, BULLET_COUNT);
            event.add(type, MAGAZINE_CAPACITY);
            event.add(type, RELOAD_TIME);
            
            // 添加近战相关属性
            event.add(type, MELEE_DAMAGE);
            event.add(type, MELEE_DISTANCE);
            
            // 添加过热体系属性
            event.add(type, HEAT_MAX);
            event.add(type, HEAT_COOLING);
            event.add(type, HEAT_COOLING_DELAY);
            event.add(type, HEAT_OVERHEAT_TIME);
        });
    }
}
