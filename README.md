...
配置修改后无需重启游戏，使用以下命令即可热重载：

```
/reload
```

热重载支持所有配置项，包括伤害计算模式、最大伤害倍率等。

### 独立属性系统

- **独立实现**: 模组使用独立的属性系统，不依赖外部技能模组
- **自动检测**: 自动检测枪械类型并应用对应的属性加成
- **灵活配置**: 支持多种伤害计算模式，可通过配置文件调整
