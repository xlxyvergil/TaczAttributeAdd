...
- **移动速度**: `taa:move_speed` - 持枪移动速度
  - **计算公式**: 最终移动速度 = 原始移动速度 + (玩家移动速度属性值 - 1.0)
  - **计算示例**: 若原始值为0.8，玩家属性为1.2，则最终值为0.8 + (1.2 - 1.0) = 1.0
...
- **过热上限**: `taa:heat_max` - 满能量值（过热上限）倍率
  - **计算公式**: 最终过热上限 = 原始过热上限 × 玩家过热上限属性值
  - **计算示例**: 若原始上限为100，玩家属性为2.0，则最终上限为100 × 2.0 = 200
  - **关联影响**: 同步作用于蓄热锁定阈值、射速热惩罚与热散射的百分比、热量条显示的分母
- **散热速度**: `taa:heat_cooling` - 散热量随停火时长增长的速度倍率
  - **计算公式**: 实际散热值 = (elapsed/10000) × 原始冷却倍率 × 玩家散热速度属性值
  - **计算示例**: 若原始冷却倍率为15，玩家属性为2.0，则实际散热倍率为30
- **冷却延迟**: `taa:heat_cooling_delay` - 停火后多久开始散热（毫秒）的倍率
  - **计算公式**: 最终冷却延迟 = (long)(原始冷却延迟 × 玩家冷却延迟属性值)
  - **计算示例**: 若原始延迟为1000ms，玩家属性为0.5，则最终延迟为500ms
- **锁枪时间**: `taa:heat_overheat_time` - 完全过热后的锁枪时长（毫秒）的倍率
  - **计算公式**: 最终锁枪时间 = (long)(原始锁枪时间 × 玩家锁枪时间属性值)
  - **计算示例**: 若原始锁枪时间为3000ms，玩家属性为0.5，则最终为1500ms

> **说明**: 过热属性为实体动态乘法倍率，在消费点即时计算（不经过属性缓存），同时覆盖 Java 原生蓄热/散热路径与 Lua 脚本路径（`tick_heat` / `handle_shoot_heat`），两条路径互斥不叠加。
